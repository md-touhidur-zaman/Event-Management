...
export default function AdminEventDetailsModal({
  open,
  setOpen,
  event,
            ...
            <div>
                ...
                <div className="flex items-center gap-3">
                    <div className="relative w-20 h-20 rounded-full border flex justify-center items-center">
                        <Image className="p-2 rounded-full" src={event?.host.user.picture} alt="profile" fill style={{objectFit: "contain"}}/>
                    </div>
                    <div>
                        <p>Name: {event?.host.user.name}</p>
                        <p>Email: {event?.host.user.email}</p>
                        <p>Phone: {event?.host.user.phone}</p>
                    </div>
                </div>
